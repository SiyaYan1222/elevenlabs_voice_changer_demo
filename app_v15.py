...
def normalize_idv_url(raw_url: str) -> str:
...
def set_idv_url(raw_url: str):
    url = normalize_idv_url(raw_url)
    return url, render_idv_iframe(url)
...
@media (max-width: 760px) {
  ...
  .market-grid { grid-template-columns: 1fr; }
  .idv-embed-topline { flex-direction: column; }
  .idv-embed-frame { min-height: 680px; }
}
...
with gr.Blocks(title="Voice Changer Lab", css=CUSTOM_CSS) as demo:
    ...
    with gr.Tabs():
        ...
        with gr.Tab("4. IDV Verification"):
            gr.Markdown(
                "Embed the separately running TitaNet IDV app by URL. Use the LAN IP when opening this Voice Changer app from another device on the same Wi-Fi.",
                elem_classes="mode-note",
            )
            with gr.Row():
                idv_lan_btn = gr.Button(f"Use LAN default ({DEFAULT_IDV_LAN_URL})")
                idv_local_btn = gr.Button(f"Use local default ({DEFAULT_IDV_LOCAL_URL})")
            with gr.Row():
                idv_url_input = gr.Textbox(
                    label="IDV app address",
                    value=DEFAULT_IDV_LAN_URL,
                    placeholder="Example: 192.168.242.249:7860",
                    scale=4,
                )
                idv_load_btn = gr.Button("Load IDV app", variant="primary", scale=1)
            idv_embed = gr.HTML(value=render_idv_iframe(DEFAULT_IDV_LAN_URL))
            idv_lan_btn.click(
                fn=lambda: set_idv_url(DEFAULT_IDV_LAN_URL),
                inputs=[],
                outputs=[idv_url_input, idv_embed],
            )
            idv_local_btn.click(
                fn=lambda: set_idv_url(DEFAULT_IDV_LOCAL_URL),
                inputs=[],
                outputs=[idv_url_input, idv_embed],
            )
