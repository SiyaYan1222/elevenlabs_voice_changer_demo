...
def tts_to_wav_file(
    ...
    This makes merging chunks easy later.
    """
    ...
    wav_path = write_wav_from_pcm_bytes(response.content, SAMPLE_RATE)
    return wav_path
...
with gr.Blocks(title="ElevenLabs Continuous Voice Changer Demo") as demo:
    gr.Markdown(
        ...
        2. **Continuous microphone** — listens locally, auto-detects speech end, then outputs cloned voice sentence by sentence.

        Keep sentences short for lower delay.
        """
    )
    ...
    with gr.Tab("Continuous Microphone"):
        ...
        latest_transcript = gr.Textbox(
            label="Latest Transcript",
            lines=3,
        )

        merged_audio = gr.Audio(
            label="Merged Final Audio",
            type="filepath",
        )
        ...
        # If your Gradio version does not support Timer, comment out these 3 lines
